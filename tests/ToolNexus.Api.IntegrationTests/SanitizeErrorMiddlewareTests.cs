...
public sealed class SanitizeErrorMiddlewareTests
{
    ...
    public async Task InvokeAsync_DoesNotLeakInternalExceptionMessage()
    {
        ...
        var response = await JsonSerializer.DeserializeAsync<SanitizeErrorMiddleware.ApiErrorResponse>(context.Response.Body, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
