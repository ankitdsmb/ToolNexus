...
- Kernel lifecycle compliance upgraded for `case-converter` and `text-diff` with explicit `create(root)`, `init()`, and `destroy()` support.
- Keyboard shortcut ownership for `case-converter` and `text-diff` is now managed by `KeyboardEventManager` instead of direct global listeners.
- Added frontend lifecycle + isolation tests for `case-converter` and `text-diff`, including remount stress (`x50`) and cleanup assertions.
...
## Current Metrics
- Kernelized tools in this incremented baseline: `base64-decode`, `url-encode`, `case-converter`, `text-diff`.
