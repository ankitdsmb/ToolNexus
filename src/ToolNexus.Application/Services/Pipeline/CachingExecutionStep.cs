    ...
    ILogger<CachingExecutionStep> logger) : IToolExecutionStep
{
    ...
    {
        var cacheKey = BuildCacheKey(context.ToolId, context.Action, context.Input);
    ...
    private static string BuildCacheKey(string toolId, string action, string input)
    {
        var normalizedToolId = toolId.Trim().ToLowerInvariant();
        var normalizedAction = action.Trim().ToLowerInvariant();
        var hashBytes = SHA256.HashData(Encoding.UTF8.GetBytes(input));
        return $"{normalizedToolId}:{normalizedAction}:{Convert.ToHexString(hashBytes).ToLowerInvariant()}";
    }
}
